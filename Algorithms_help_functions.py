...
def send_named_message_to(receiver, self_agent, message, message_type='message'):
    ...
    receiver.get_access_to_named_inbox(message_type, self_agent.get_name(), message)
...
def go_acording_to_HPA(agent, curr_pos, new_pos, curr_nei):
    curr_inbox = agent.get_access_to_inbox('copy')
    num_of_agent = agent.get_num_of_agent()
    for nei in curr_nei:
        num_of_nei = nei.get_num_of_agent()
        if num_of_agent > num_of_nei:
            if new_pos == curr_inbox[num_of_nei][-1]:
                return curr_pos
    return new_pos
...
def wait_to_receive_certain_named(agent, list_of_senders, order_of_message):
    ...
    while not_received:
        ...
        for curr_nei in list_of_senders:
...
def calculate_pos_for_MSHPA(agent, possible_pos, for_alg):
    inbox = agent.get_access_to_inbox('copy')
    num_of_agent = agent.get_num_of_agent()
    curr_robot_nei = agent.get_curr_robot_nei()
    order_of_message = 1
...
#     curr_target_nei = agent.get_curr_nei()
#     curr_robot_nei = agent.get_curr_robot_nei()
#
#     # for Targets
#     for nei in curr_target_nei:
#         if nei is agent:
#             print('[ERROR]: agent is self_agent inside self_agent.get_curr_nei()!')
